...
小度智能家居自定义集成（当前版本 **v0.9.2**）：让 Home Assistant 中的设备可以被
...
- **单一中枢设备（v0.9.2）**：对齐 HA 桥接型集成惯例（如 HomeKit Bridge），
  设备注册表中只保留一个「小度中枢」虚拟设备条目，作为集成页折叠展开与
  设备计数的锚点；被桥接的 HA 设备不再镜像进注册表（避免与设备真实集成
  如 Xiaomi Home 重复展示）。升级后旧版镜像条目会在下次集成加载时自动
  清理。设备的总览与能力配置仍在集成项 → 「设备与能力」选项流程中完成，
  设备行不再注入「单元与能力」前置模块。
